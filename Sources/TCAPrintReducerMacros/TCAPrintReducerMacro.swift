...
public struct TCAPrintReducerMacro: MemberMacro {
    ...
    public static func expansion(
        ...
        let subsystemArg =
        arguments?.first(where: { $0.label?.text == "subsystem" })?.expression.description
        ?? "\"default.subsystem\""
        let categoryArg =
        arguments?.first(where: { $0.label?.text == "category" })?.expression.description
        ?? "\"default.category\""
        let levelArg =
        arguments?.first(where: { $0.label?.text == "level" })?.expression.description ?? ".default"
...
struct TCAPrintReducerPlugin: CompilerPlugin {
    let providingMacros: [Macro.Type] = [
        ...
        EasyMappableMacro.self,
        InMemoryContainerMacro.self,
    ]
}
